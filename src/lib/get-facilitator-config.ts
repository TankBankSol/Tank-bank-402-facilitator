...
export function getFacilitatorConfig(): FacilitatorConfig {
  ...
  try {
    config = FacilitatorConfigSchema.parse({
      ...
      tankBankWallet: process.env.TANK_BANK_WALLET,
      solanaRpcUrl: process.env.SOLANA_RPC_URL ?? 'https://mainnet.helius-rpc.com/?api-key=YOUR_API_KEY_HERE',
      solanaWsUrl: process.env.SOLANA_WS_URL ?? 'wss://mainnet.helius-rpc.com/?api-key=YOUR_API_KEY_HERE',
